...
from arcana.exceptions import (
    ...
    ArcanaCantPickleAnalysisError, ArcanaUsageError, ArcanaError,
...
class Analysis(object):
    ...
    inputs : Dict[str, FilesetFilter | FilesetSpec | FieldFilter | FieldSpec] |
             List[FilesetFilter | FilesetSpec | FieldFilter | FieldSpec]
        Either a list or a dictionary containing FilesetFilter,
        FieldFilter, FilesetSpec, or FieldSpec objects, which specify the
        names of input filesets to the analysis, i.e. those that won't
        be generated by this analysis (although can be derived by the parent
        MultiAnalysis)
    ...
    add_data_specs : List[FilesetSpec|FieldSpec]
        ...
        used and generated by the analysis to (Fileset|Field)Spec objects.
    add_param_specs : List[ParamSpec]
        ...
        provided to pipelines in the analysis
    """
    ...
    def __init__(self, name, dataset, processor, inputs,
        ...
        try:
            ...
            if not issubclass(metaclass, AnalysisMetaClass):
                raise KeyError
        except KeyError:
            ...
            for inpt_name, inpt in list(inputs.items()):
                if isinstance(inpt, basestring):
                    ...
                    if spec.is_fileset:
                        ...
                        inpt = FieldFilter(inpt_name, pattern=inpt,
                                           dtype=spec.dtype,
                                           is_regex=True)
                    inputs[inpt_name] = inpt
        # Check validity of analysis inputs
        ...
        try:
            ...
            try:
                ...
                raise ArcanaDesignError(
                    ...
                    "present in '{}' analysis".format(getter_name, self))
    ...
    def __reduce__(self):
        """
        Control how analysis classes are pickled to allow some generated
        ...
        try:
            # Check whether the analysis class is generated or not by
                ...
                if isinstance(attr, types.FunctionType):
                    try:
                        if not attr.auto_added:
                            raise ArcanaCantPickleAnalysisError()
                    except (AttributeError, ArcanaCantPickleAnalysisError):
                        raise ArcanaCantPickleAnalysisError(
                            "Cannot pickle auto-generated analysis class "
    ...
    @property
    def prefix(self):
        """The analysis name as a prefix for fileset names"""
        return self.name + '_'

    @property
    def name(self):
        """Accessor for the unique analysis name"""
        return self._name
    ...
    def spec(self, name):
        ...
        been passed to the analysis as an input or can be derived.
    ...
    def bound_spec(self, name):
        """
        Returns an input selector or derived spec bound to the analysis, i.e.
        ...
        try:
            ...
            if not spec.derived and spec.default is None:
                raise ArcanaMissingDataException(
                    ...
                    "was not supplied when the analysis '{}' was "
    ...
    @classmethod
    def data_specs(cls):
        """Lists all data_specs defined in the analysis class"""
    ...
    def data_spec_names(cls):
        """Lists the names of all data_specs defined in the analysis"""
        return iter(cls._data_specs.keys())

    @classmethod
    def param_spec_names(cls):
        """Lists the names of all param_specs defined in the analysis"""
    ...
    def acquired_data_specs(cls):
        """
        Lists all data_specs defined in the analysis class that are
        provided as inputs to the analysis
        """
    ...
    def derived_data_specs(cls):
        """
        Lists all data_specs defined in the analysis class that are typically
    ...
    def derived_data_spec_names(cls):
        """Lists the names of generated data_specs defined in the analysis"""
        return (c.name for c in cls.derived_data_specs())

    @classmethod
    def acquired_data_spec_names(cls):
        "Lists the names of acquired data_specs defined in the analysis"
    ...
    def prov(self):
        """
        Extracts provenance information from the analysis for storage alongside
...
class AnalysisMetaClass(type):
    """
    Metaclass for all analysis classes that collates data specs from
    ...
    def __new__(metacls, name, bases, dct):  # @NoSelf @UnusedVariable
        if not any(issubclass(b, Analysis) for b in bases):
            raise ArcanaUsageError(
                "AnalysisMetaClass can only be used for classes that "
                "have Analysis as a base class")
        try:
        ...
        # Append description of Analysis parameters to class
        try:
            docstring = dct['__doc__']
        except KeyError:
            docstring = '{} Analysis class'.format(name)
        if 'Parameters' not in docstring:
            docstring += Analysis.__doc__
