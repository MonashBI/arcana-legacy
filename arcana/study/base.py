...
class Study(object):
    ...
    inputs : Dict[str, InputFileset | FilesetSpec | InputField | FieldSpec] | List[InputFileset | FilesetSpec | InputField | FieldSpec]
        Either a list or a dictionary containing InputFileset,
        InputField, FilesetSpec, or FieldSpec objects, which specify the
        ...
        if not isinstance(inputs, dict):
            ...
            for inpt_name, inpt in list(inputs.items()):
                if isinstance(inpt, basestring):
                    ...
                    if spec.is_fileset:
                        inpt = InputFileset(inpt_name, pattern=inpt)
                    else:
                        inpt = InputField(inpt_name, pattern=inpt,
                                             dtype=spec.dtype)
