...
class Study(object):
    ...
    add_parameter_specs : List[ParameterSpec]
        Adds data specs to the '_parameter_specs' class attribute,
        which is a dictionary that maps the names of datasets that are
        used and generated by the study to ParameterSpec objects.
    """
...
#             else:
        ...
        # Record parameters accessed before a pipeline is created
        # so they can be attributed to the pipeline after creation
        self._pre_parameters = defaultdict(list)
        ...
        self._parameters = {}
        # Set parameters
        for opt_name, opt in list(parameters.items()):
            if not isinstance(opt, Parameter):
                opt = Parameter(opt_name, opt)
            try:
                opt_spec = self._parameter_specs[opt_name]
            except KeyError:
                raise ArcanaNameError(
    ...
    def pre_parameter(self, name, pipeline_name, name_prefix='', **kwargs):  # @UnusedVariable @IgnorePep8
        """
        Retrieves the value of the parameter provided to the
        study and "pre-registers" the parameter as being used by the
        pipeline matching the name provided.

        It is used to access parameters that before a pipeline is created
        ...
        pipeline_name : str
            The name of the pipeline to attribute the parameter to
        """
        parameter = self._get_parameter(name)
        # Register parameter as being used by the pipeline
        self._pre_parameters[name_prefix + pipeline_name].append(parameter)
        return parameter.value

    @property
    def parameters(self):
        for opt_name in self._parameter_specs:
            yield self._get_parameter(opt_name)
