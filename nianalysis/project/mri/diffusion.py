...
from .structural import T2Project
...
class DiffusionProject(T2Project):
    ...
    def preprocess_pipeline(self, phase_dir='LR', **kwargs):  # @UnusedVariable @IgnorePep8
        ...
        # Create nodes to convert preprocessed dataset and gradients to FSL format
    ...
    def brain_mask_pipeline(self, mask_tool='bet', **kwargs):  # @UnusedVariable @IgnorePep8
        ...
        """
        if mask_tool == 'fsl':
            pipeline = super(DiffusionProject, self).brain_mask_pipeline(
                **kwargs)
    ...
    def extract_b0_pipeline(self, **kwargs):  # @UnusedVariable
        """
        Extracts the b0 images from a DWI project and takes their mean
        """
        pipeline = self._create_pipeline(
            name='extract_b0',
            inputs=['dwi_preproc', 'grad_dirs', 'bvalues'],
            outputs=['mri_scan'],
            description="Extract b0 image from a DWI project",
    ...
    # The list of project components that are either acquired from the datasetner
    # (i.e. without a specified pipeline) or generated by processing pipelines
    _components = _create_component_dict(
        Dataset('dwi_scan', mrtrix_format),
        Dataset('forward_rpe', mrtrix_format),
        Dataset('reverse_rpe', mrtrix_format),
        Dataset('mri_scan', nifti_gz_format, extract_b0_pipeline),
        Dataset('tensor', nifti_gz_format, tensor_pipeline),
        Dataset('fa', nifti_gz_format, tensor_pipeline),
        Dataset('adc', nifti_gz_format, tensor_pipeline),
        Dataset('fod', mrtrix_format, tensor_pipeline),
        Dataset('dwi_preproc', nifti_gz_format, preprocess_pipeline),
        Dataset('bias_correct', nifti_gz_format, bias_correct_pipeline),
        ...
        Dataset('tbss_skeleton_mask', nifti_gz_format, tbss_pipeline,
             multiplicity='per_project'),
        inherit_from=T2Project.generated_components())


class NODDIProject(DiffusionProject):
    ...
    _components = _create_component_dict(
        Dataset('low_b_dw_scan', mrtrix_format),
        Dataset('high_b_dw_scan', mrtrix_format),
        Dataset('forward_rpe', mrtrix_format),
        Dataset('reverse_rpe', mrtrix_format),
        Dataset('dwi_scan', mrtrix_format, concatenate_pipeline),
        Dataset('ficvf', nifti_format, noddi_fitting_pipeline),
        Dataset('odi', nifti_format, noddi_fitting_pipeline),
        Dataset('fiso', nifti_format, noddi_fitting_pipeline),
        Dataset('fibredirs_xvec', nifti_format, noddi_fitting_pipeline),
        Dataset('fibredirs_yvec', nifti_format, noddi_fitting_pipeline),
        Dataset('fibredirs_zvec', nifti_format, noddi_fitting_pipeline),
        Dataset('fmin', nifti_format, noddi_fitting_pipeline),
        Dataset('kappa', nifti_format, noddi_fitting_pipeline),
        Dataset('error_code', nifti_format, noddi_fitting_pipeline),
        inherit_from=DiffusionProject.generated_components())
